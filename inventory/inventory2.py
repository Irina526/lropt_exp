...
def trainloop(r1,foldername):
    ...
    for N in np.array([50,100,300,500,1000,1500,2000,3000,4000,5000]):
        ...
        # solve
        # seed 1, 
        result = prob.train(lr = 0.001,num_iter=1000, optimizer = "SGD", seed = seed, init_A = 0.8*init, init_b = 0.8*init_bval, init_lam = 1, init_mu = 1.5, mu_multiplier=1.01, init_alpha = -0.0, test_percentage = test_p, save_history = False, lr_step_size = 100, lr_gamma = 0.2, position = False, random_init = True, num_random_init=6, parallel = True, eta = eta, kappa=0.)
        A_fin = result.A
        b_fin = result.b

        # Grid search epsilon
        result4 = prob.grid(epslst = np.linspace(0.01, 6, 300), init_A = init, init_b = init_bval, seed = seed, init_alpha = 0., test_percentage =test_p,newdata = newdata)
        dfgrid = result4.df

        result5 = prob.grid(epslst = np.linspace(0.01,6, 300), init_A = A_fin, init_b = b_fin, seed = seed, init_alpha = 0., test_percentage = test_p,newdata = newdata)
        dfgrid2 = result5.df
