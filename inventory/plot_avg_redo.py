...
for N in nvals:
    ...
    for i in range(len(etas)):
        ...
        for r in range(1,18):
            ...
            ind_s0 = np.absolute(np.mean(np.vstack(dfgrid['Avg_prob_test']),axis = 1)-0.0005).argmin()
            ind_r0 = np.absolute(np.mean(np.vstack(dfgrid2['Avg_prob_test']),axis = 1)-0.0005).argmin()
            ...
            ind_s = np.absolute(np.mean(np.vstack(dfgrid['Avg_prob_test']),axis = 1)-(etas[i]-0.002)).argmin()
            ind_r = np.absolute(np.mean(np.vstack(dfgrid2['Avg_prob_test']),axis = 1)-(etas[i]-0.002)).argmin()
    ...
    dfgrid = pd.read_csv(foldername + f"results{17}/" + f"gridmv_{N,m,0}.csv")
    ...
    print(np.mean(mro_vals,axis=0), np.mean(mro_probs,axis=0))
    print(np.mean(np.array(mro_probs) >= 0.03,axis=0))
    print(np.mean(np.array(mro_probs) >= 0.05,axis=0))
    ...
    plt.plot(np.mean(mro_probs,axis=0)[1:-1],np.mean(mro_vals, axis=0)[1:-1], label = "Wass DRO", color = "tab:green" )
    plt.fill_between(np.mean(mro_probs,axis=0)[1:-1],np.quantile(mro_vals,lower_q, axis=0)[1:-1],np.quantile(mro_vals,upper_q, axis=0)[1:-1], color = "tab:green", alpha=0.3)
    ...
    plt.vlines(ymin=-466, ymax=-456.5, x=0.028, linestyles=":",
           color="tab:red", label=r"$\hat{\eta}=0.028,0.058$") 
    plt.vlines(ymin=-466, ymax=-456.5, x=0.058, linestyles=":",
           color="tab:red") 
    plt.hlines(xmin=0.028, xmax=0.058, y=-461.37, linestyles="--",
           color="black") 
