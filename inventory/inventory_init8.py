...
def trainloop(r1,foldername):
    ...
    for N in np.array([100]):
        ...
        # solve
        # seed 1, 
        result = prob.train(lr = 0.0001,num_iter=1000, num_iter_size = 300, lr_size= 0.001, train_size = True, optimizer = "SGD", seed = seed, init_A = init, init_b = init_bval, init_lam = 2.0, init_mu =2.0, mu_multiplier=1.02, init_alpha = -0.0, test_percentage = test_p, save_history = False, lr_step_size = 50, lr_gamma = 0.5, position = False, random_init = False, num_random_init=6, parallel = True, eta = eta, kappa=0.)
        A_fin = result.A
        b_fin = result.b
        eps_fin = result.eps
