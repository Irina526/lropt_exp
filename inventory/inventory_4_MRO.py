    ...
    for N in np.array([100]):
        ...
        while not data_gen:
            try: 
                ...
                initn = np.real(sc.linalg.sqrtm(np.cov(train.T)))
                init = np.eye(m)
        ...
        u = lropt.UncertainParameter(m,uncertainty_set = lropt.MRO(K=5, p=2, data=train, train=True))
        ...
        # solve
        # seed 1, 
        result = prob.train(lr = 0.001,num_iter=1000,optimizer = "SGD", seed = seed, init_A = initn, init_b = init_bval, init_lam = 30.0, init_mu =30.0, mu_multiplier=1.02, init_alpha = -0.0, test_percentage = test_p, save_history = False, lr_step_size = 50, lr_gamma = 0.5, position = False, random_init = False, num_random_init=6, parallel = True, eta = eta, kappa=0.1)
        A_fin = result.A
        b_fin = result.b
        eps_fin = result.eps
        
        # Grid search epsilon
        result4 = prob.grid(epslst = np.linspace(0.0001, 5, 100), init_A = init, init_b = init_bval, seed = seed, init_alpha = 0., test_percentage =test_p,newdata = (newdata,new_y_data), eta=eta)
        dfgrid = result4.df

        result5 = prob.grid(epslst = np.linspace(0.0001,5, 100), init_A = A_fin, init_b = b_fin, seed = seed, init_alpha = 0., test_percentage = test_p,newdata = (newdata,new_y_data), eta=eta)
        dfgrid2 = result5.df
        prob.solve()
...
if __name__ == '__main__':
    ...
    for N in nvals:
        ...
        dfgrid = dfgrid.drop(columns=["step","Probability_violations_test","Probability_violations_train","var_values"])
        dfgrid2 = pd.read_csv(foldername +f"gridre_{N,m,0}.csv")
        dfgrid2 = dfgrid2.drop(columns=["step","Probability_violations_test","Probability_violations_train","var_values"])
        ...
        for r in range(1,R):
            ...
            newgrid = newgrid.drop(columns=["step","Probability_violations_test","Probability_violations_train","var_values"])
            dfgrid = dfgrid.add(newgrid.reset_index(), fill_value=0)
            newgrid2 = pd.read_csv(foldername +f"gridre_{N,m,r}.csv")
            newgrid2 = newgrid2.drop(columns=["step","Probability_violations_test","Probability_violations_train","var_values"])
