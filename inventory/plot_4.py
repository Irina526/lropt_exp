...
for N in nvals:
    ...
    for i in range(len(etas)):
        ...
        for method in range(6):
            probs_beta[method] = []
        for r in range(20):
            dfgrid3 = pd.read_csv(foldername + f"results{i}/" + f"gridmv_{N,m,r}.csv")
            dfgrid2 = pd.read_csv(foldername + f"results{0}/" + f"gridre_{N,m,r}.csv")
    ...
    for ind_val in range(len(testetas)):
        ...
        print(testetas[ind_val], min_ind)
    ...
    plt.title(f"$m={m}$")
    plt.legend()
    plt.savefig(foldername + f"{m}_{N}.pdf", bbox_inches='tight')
    plt.show()